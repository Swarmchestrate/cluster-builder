...
class PostgresConfig:
    ...
    def from_dict(cls, config: dict[str, str]) -> "PostgresConfig":
        ...
        if missing_keys:
            logger.error(
                f"Missing required PostgreSQL configuration keys: {', '.join(missing_keys)}"
            )
            raise ValueError(
        ...
        logger.info(
            f"Creating PostgresConfig from dict with user={config.get('user')} host={config.get('host')} database={config.get('database')}"
        )
        return cls(
            user=config["user"],
    ...
    def from_env(cls) -> "PostgresConfig":
        ...
        logger.info(
            f"Creating PostgresConfig from environment with user={os.environ.get('POSTGRES_USER')} host={os.environ.get('POSTGRES_HOST')} database={os.environ.get('POSTGRES_DATABASE')}"
        )
        return cls(
