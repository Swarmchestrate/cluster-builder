...
if systemctl is-active --quiet k3s; then
    ...
    if [[ "${ha}" == "true" ]]; then
        ...
        curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--cluster-init --node-external-ip=${public_ip} --node-name="${resource_name}" --node-label=labels.swarmchestrate.eu/ms_id=${resource_name} --flannel-backend=wireguard-native --flannel-external-ip" K3S_TOKEN="${k3s_token}" sh -s - server
    else
        log_message "Installing in single-server mode..."
        curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--node-external-ip=${public_ip} --node-name="${resource_name}" --node-label=labels.swarmchestrate.eu/ms_id=${resource_name} --flannel-backend=wireguard-native --flannel-external-ip" K3S_TOKEN="${k3s_token}" sh -s - server
    fi
