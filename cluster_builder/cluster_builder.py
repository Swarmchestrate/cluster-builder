...
class Swarmchestrate:
    def __init__(
        ...
        logger.info(f"Initialized Swarmchestrate with template_dir={template_dir}, output_dir={output_dir}")
    ...
    def prepare_modules(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            error_msg = f"Failed to create directory {os.path.dirname(main_tf_path)}: {e}"
            logger.error(error_msg)
        ...
        try:
            ...
            logger.info(f"Copied user data template from {user_data_src} to {user_data_dst}")
    ...
    def destroy(self, cluster_name: str) -> None:
        ...
        try:
            ...
            error_msg = f"Error during destruction of cluster '{cluster_name}': {e.stderr}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
        except Exception as e:
